...
from cargar_imagen import create_folder, save_and_resize, comparativa_redim
...
print("Tiempo de ejecución redimension con",
      str(n_hilos), "hilos: ", str(t1), "s")
print('Finalizando preprocesado de imagenes.....')

# Comprobamos que cumplen el mismo orden originales y redimensionadas
comparativa_redim('dataset/Data Images', 'dataset/Data_Images64x64')

print("Comienzo del ordenamiento de datos")
dato_entrada, t_num = cargar_csv('dataset/Data.csv', 'dataset/Data Images', 'dataset/Data_Images64x64', {
...
for cv in CV:
    for split in splits:
        for leaf in leafs:
            ...
            scoresDTC['DTC_CV' + str(cv) + '_splits' + str(split) + '_leafs' + str(leaf)] = dtc(
                X=X_1D_reduced, Y=t_num, samples_split=split, samples_leaf=leaf, cv=cv)
...
for cv in CV:
    for tree in trees:
        ...
        scoresRF['RF_CV' + str(cv) + '_trees' + str(tree)
                 ] = rf(X=X_1D_reduced, Y=t_num, trees=tree, cv=cv)
...
for cv in CV:
    for neighbour in neighbours:
        ...
        scoresKNN['KNN_CV' + str(cv) + '_neigh' + str(neighbour)] = knn(
            X=X_1D_reduced, Y=t_num, neighbours=neighbour, cv=cv)
...
for cv in CV:
    for convolucion in conNeurons:
        for densa in denseNeurons:
            print('Con CV =', cv, ', conNeurons =',
                  conNeurons, 'y denseNeurons =', denseNeurons)
            scoresDL['DL_CV' + str(cv) + '_Conv' + str(red)] = conv(X=X_3Dnorm, Y=t_num, cv=cv, conNeurons=convolucion,
                                                                    denseNeurons=densa, ler=ler,  batch=batch, epochs=epochs, resolucion=resolucion)
            red += 1
...
contraste(accuracy1=scoresDTC[opcionDTC]['accuracy'], accuracy2=scoresKNN[opcionKNN]['accuracy'],
          accuracy3=scoresRF[opcionRF]['accuracy'], accuracy4=scoresDL[opcionDL]['accuracy'], alpha=0.1)
